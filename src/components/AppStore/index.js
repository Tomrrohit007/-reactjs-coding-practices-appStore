...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterList = () => {
      return categoryList().filter(each =>
        each.appName.toLowerCase().includes(search.toLocaleLowerCase()),
      )
    }
        ...
        <ul className="tab-item">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              activeTabColor={eachTab.tabId === activeTab}
              key={eachTab.tabId}
            />
          ))}
        </ul>
